...
module.exports = {
	name: 'invert',
	description: 'generate invert image',
	...
	async execute(interaction) {
		...
		try {
			const buffer = await canvacord.Canvas.invert(query.displayAvatarURL({ size: 512, dynamic: true, format: 'png' }));
			const attachment = new Discord.MessageAttachment(buffer, 'invert.png');
			embed.setAuthor({ name: `${interaction.client.user.displayName} | Invert`, iconURL: `${interaction.client.user.displayAvatarURL()}` });
