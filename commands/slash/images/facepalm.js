...
module.exports = {
	name: 'facepalm',
	description: 'generate facepalm image',
	...
	async execute(interaction) {
		...
		try {
			const buffer = await canvacord.Canvas.facepalm(query.displayAvatarURL({ size: 512, dynamic: true, format: 'png' }));
			const attachment = new Discord.MessageAttachment(buffer, 'facepalm.png');
			embed.setAuthor({ name: `${interaction.client.user.displayName} | Facepalm`, iconURL: `${interaction.client.user.displayAvatarURL()}` });
			embed.setColor(config.app.color);
			embed.setImage('attachment://facepalm.png');
			embed.setTimestamp();
